...
class Migrate extends CI_Controller
{
 ...
 public function to_version($version_number)
 {
 $this->load->library('migration');
...
class Migrate extends CI_Controller {
	...
	public function index()
	{
		...
		/*
		$data['assets'] = [
		'bootstrap_css' => base_url('assets/custom/migrate/css/bootstrap.min.css'),
		'bootstrap_js' => base_url('assets/custom/migrate/js/bootstrap.min.js'),
		'jquery' => base_url('assets/custom/migrate/js/jquery-2.2.4.min.js'),
		];
		*/
			...
			elseif (array_key_exists($version, $this->migrations))
			{
				...
				elseif ($v === FALSE)
				{
					$response = [
						'type' => 'danger',
						'header' => 'Error!',
						'content' => 'Migration failed. '.$this->migrations->error_string().' '.$this->db->_error_message(),
					];
				}
			}
			else
